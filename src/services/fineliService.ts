import { FoodItem } from '../types';
import rawFineliData from '../data/fineli_foods.json';
import { normalizeText } from './ciqualService';

export interface FineliFoodRaw {
  id: string;
  code: string;
  name: string;
  servingSizeG: number;
  calories100g: number;
  proteins100g: number;
  carbs100g: number;
  fats100g: number;
  fiber100g?: number;
  sodiumMg100g?: number;
  source: 'FINELI';
}

const fineliFoods: FoodItem[] = (rawFineliData as FineliFoodRaw[]).map((item) => ({
  id: item.id,
  name: item.name,
  brand: 'Fineli (FI/EN)',
  servingSizeG: item.servingSizeG || 100,
  calories100g: item.calories100g,
  proteins100g: item.proteins100g,
  carbs100g: item.carbs100g,
  fats100g: item.fats100g,
  fiber100g: item.fiber100g,
  sodiumMg100g: item.sodiumMg100g,
  source: 'FINELI',
  createdAt: 0,
}));

/**
 * Searches the offline Fineli English dataset for matching food items
 */
export function searchFineliFoods(query: string, limit = 30): FoodItem[] {
  const normQuery = normalizeText(query);
  if (!normQuery || normQuery.length < 2) {
    return [];
  }

  const queryWords = normQuery.split(/\s+/).filter(Boolean);
  const scoredResults: { item: FoodItem; score: number }[] = [];

  for (let i = 0; i < fineliFoods.length; i++) {
    const item = fineliFoods[i];
    const normName = normalizeText(item.name);

    let score = 0;

    // Direct string match checks on Fineli name
    if (normName === normQuery) {
      score += 100;
    } else if (normName.startsWith(normQuery + ' ') || normName.startsWith(normQuery + ',')) {
      score += 80;
    } else if (normName.startsWith(normQuery)) {
      score += 65;
    } else if (normName.includes(normQuery)) {
      score += 45;
    }

    // Word coverage matching
    let allWordsMatch = true;
    let wordScore = 0;

    for (const word of queryWords) {
      if (normName.includes(word)) {
        wordScore += 15;
      } else {
        allWordsMatch = false;
      }
    }

    if (allWordsMatch) {
      score += wordScore + 25;
    } else if (wordScore > 0) {
      score += wordScore;
    }

    if (score > 0) {
      const concisenessRatio = Math.min(1, normQuery.length / Math.max(normName.length, 1));
      score += Math.round(concisenessRatio * 20);

      scoredResults.push({ item, score });
    }
  }

  scoredResults.sort((a, b) => b.score - a.score);

  return scoredResults.slice(0, limit).map((r) => r.item);
}

/**
 * Retrieves a single Fineli food item by its ID
 */
export function getFineliFoodById(id: string): FoodItem | undefined {
  return fineliFoods.find((f) => f.id === id);
}

/**
 * Returns total count of Fineli food items
 */
export function getFineliFoodsCount(): number {
  return fineliFoods.length;
}
